...
class VideoNumbersWithActions:
	...
	def _give_ob(self):
		# Draws internal number and last action on image, number determines also background color for world model
		# convenience
		text_to_draw = 'S{:02}\nA{}'.format(self.state['number'], self.state['last_action'])
		im = Image.fromarray(np.full([28, 28, 3], self.state['number'], dtype=np.uint8), mode='RGB')
		draw = ImageDraw.Draw(im)
...
class ActionBackground:
	def __init__(self, arena_size=50):
		self.state = dict()
		self.action_to_RGB = [np.array([0,0,255]), np.array([0,255,0]), np.array([255,0,0]), np.array([100,100,100])]
	...
	def _give_ob(self):
		...
		ob = np.full([self.arena_size, self.arena_size, 3], background, dtype=np.uint8)
		return ob
